...
import { setUser, useChangePasswordMutation, useLoginRequestMutation, useSendCodeMutation } from '../User.slice';
...
const Recover = () => {
    ...
    const onSubmitPassword = async (data: PasswordConfirmation) => {
        console.log(data);
        console.log(user);
        if (user)
            await changePassword({
                ...
                .then(responce => {
                    console.log(responce);
                    setUserData({ email: user?.email, password: data.password })
                    setStage(4)
    ...
    const switcher = () => {
        switch (stage) {
                ...
                return (
                    <>
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className={`flex flex-col p-7 gap-4 border-none text-white mx-auto my-auto w-full`}>
                            ...
                            <button type="submit" className={`${SendCodeLoading && 'button_loading'} yellow_btn`}>
                                Continue
                            </button>
                        </form>
                    </>)
            ...
            case 3:
                return (
                    <>
                        ...
                        <form onSubmit={handleSubmitPassword(onSubmitPassword)} className={`flex flex-col p-7 gap-4 border-none text-white mx-auto my-auto w-full`}>

                            <div className="form-group flex flex-col">
                                <label className=" text-md  after:content-['*'] after:inline after:text-yellow after:font-beausans mb-1 ">
                                    Password
                                </label>

                                <div className="invalid-feedback text-red-500 text-sm mb-2" >{PasswordErrors.password?.message}</div>

                                <div className={` form-control  ${PasswordErrors.password ? 'is-invalid border border-rose-500' : 'border-none'} rounded-lg bg-lightGray flex flex-row  items-center justify-between overflow-hidden`}>

                                    <input
                                        type={passwordShown ? "text" : "password"}
                                        {...registerPassword('password')}
                                        className={`form-control focus:outline-0 focus:ring-transparent ${PasswordErrors.password ?? 'is-invalid'} text-lg bg-inherit border-none p-2 px-3 w-full font-chakra`}
                            ...
                            <div className="form-group flex flex-col">
                                ...
                                <div className="invalid-feedback text-red-500 text-sm mb-2">{PasswordErrors.confirmPassword?.message}</div>

                                <input
                                    type="password"
                                    {...registerPassword('confirmPassword')}
                                    className={`${PasswordErrors.confirmPassword ? 'is-invalid border border-rose-500' : 'border-none'}form-control focus:outline-0 focus:ring-transparent focus:border-rose-500 text-lg rounded-lg bg-lightGray p-2 px-3 font-chakra`}
                                    placeholder='Password'
                                />
            ...
            case 4:
                return (
                    <SuccessAction h1='The password has been updated' button='Log in to your account' buttonAction={onButtonLogIn} />
                )
